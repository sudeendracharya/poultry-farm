...
class AddUserRoles extends StatefulWidget {
  AddUserRoles({Key? key, required this.reFresh}) : super(key: key);

  final ValueChanged<int> reFresh;

  static const routeName = '/AddUserRoles';

  @override
  _AddUserRolesState createState() => _AddUserRolesState();
}

class _AddUserRolesState extends State<AddUserRoles> {
  ...
  void getWarehouseList(var data) {
    fetchCredientials().then((token) {
      Provider.of<InfrastructureApis>(context, listen: false)
  ...
  void getWarehouseSectionList(var data) {
    fetchCredientials().then((token) {
      Provider.of<InfrastructureApis>(context, listen: false)
  ...
  Future<void> save() async {
    ...
    Provider.of<Apicalls>(context, listen: false).tryAutoLogin().then((value) {
      ...
      Provider.of<AdminApis>(context, listen: false)
          .addUserRoles(temp, token)
          .then((value) {
        if (value == 200 || value == 201) {
          widget.reFresh(100);
          Get.back();
          successSnackbar('User Role has been added successfully');
        }
      });
    });
  }
